...
class AlexNet(nn.Module):
    def __init__(self, profile_len=1024, param_dim=18):
        super().__init__()
        # 廓形分支 (2通道 x 1024)
        # AlexNet风格,车轮、钢轨廓形对应两个通道,1D图像
        self.profile_branch = nn.Sequential(
            # nn.Conv1d(2, 32, kernel_size=11, stride=4, padding=2),
            ...
            # nn.MaxPool1d(3, stride=2),
            # nn.Flatten()
            nn.Conv1d(2, 64, kernel_size=11, stride=4, padding=2),  # 64
            nn.BatchNorm1d(64), nn.ReLU(inplace=True),
            nn.MaxPool1d(3, stride=2),
            nn.Conv1d(64, 128, kernel_size=5, padding=2),  # 128
            nn.BatchNorm1d(128), nn.ReLU(inplace=True),
            nn.MaxPool1d(3, stride=2),
            nn.Conv1d(128, 256, kernel_size=3, padding=1),  # 256
            nn.BatchNorm1d(256), nn.ReLU(inplace=True),
            nn.MaxPool1d(3, stride=2),
            nn.Conv1d(256, 512, kernel_size=3, padding=1),  # 512
            nn.BatchNorm1d(512), nn.ReLU(inplace=True),
            nn.MaxPool1d(3, stride=2),
            nn.Conv1d(512, 512, kernel_size=3, padding=1),  # 512
            nn.BatchNorm1d(512), nn.ReLU(inplace=True),
            nn.AdaptiveAvgPool1d(1),  # 全局池化，输出固定512维
            nn.Flatten()
        )
        # 参数分支 (18维)
        ...
        with torch.no_grad():
            ...
            profile_feat = self.profile_branch(dummy_profile)  # 实际输出: [1, 3968]
            param_feat = self.param_branch(dummy_param)  # 实际输出: [1, 128]
            fusion_in_dim = profile_feat.size(1) + param_feat.size(1)  # = 4096
...
def train_model(model, train_loader, val_loader, num_epochs=250):
    ...
    patience_counter = 0
    best_model_path = f"alex_best_model_{timestamp}.pth"
...
if __name__ == "__main__":
    ...
    print("=" * 33 + "AlexNet网络建立完毕" + "=" * 33)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    alex_model = AlexNet().to(device)
    print("=" * 33 + "网络权重初始化完毕" + "=" * 33)
    alex_model.apply(lambda m: init_param(m))
    print(f"Alex网络结构如下:{alex_model}")
    total, trainable = get_param_number(alex_model)
    print(f"模型总参数量:{total}, 模型可训练参数量:{trainable}")
    # 训练模型
    train_model(alex_model, train_loader, val_loader, num_epochs=250)
