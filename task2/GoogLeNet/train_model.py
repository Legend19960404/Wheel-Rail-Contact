...
class InceptionModule1D(nn.Module):
    """
    一维Inception模块，模仿GoogLeNet的核心思想
    包含并行的多个分支：1x1卷积、3x3卷积、5x5卷积、池化后1x1卷积
    """
    def __init__(self, in_channels, ch_1x1, ch_3x3_reduce, ch_3x3, ch_5x5_reduce, ch_5x5, pool_proj):
        ...
        self.branch1 = nn.Sequential(
            nn.Conv1d(in_channels, ch_1x1, kernel_size=1),
            nn.BatchNorm1d(ch_1x1),
            nn.ReLU(inplace=True)
        )

        # 3x3卷积分支
        self.branch2 = nn.Sequential(
            nn.Conv1d(in_channels, ch_3x3_reduce, kernel_size=1),
            nn.BatchNorm1d(ch_3x3_reduce),
            nn.ReLU(inplace=True),
            nn.Conv1d(ch_3x3_reduce, ch_3x3, kernel_size=3, padding=1),
            nn.BatchNorm1d(ch_3x3),
            nn.ReLU(inplace=True)
        )

        # 5x5卷积分支
        self.branch3 = nn.Sequential(
            nn.Conv1d(in_channels, ch_5x5_reduce, kernel_size=1),
            nn.BatchNorm1d(ch_5x5_reduce),
            nn.ReLU(inplace=True),
            nn.Conv1d(ch_5x5_reduce, ch_5x5, kernel_size=5, padding=2),
            nn.BatchNorm1d(ch_5x5),
            nn.ReLU(inplace=True)
        )
...
class GoogLeNet(nn.Module):
    def __init__(self, profile_len=1024, param_dim=18):
        super().__init__()
        # 廓形分支 (2通道 x 1024)
        # GoogLeNet风格,车轮、钢轨廓形对应两个通道,1D图像
        self.profile_branch = nn.Sequential(
            ...
            # 第一个Inception块
            InceptionModule1D(64, 16, 16, 64, 8, 16, 32),
            nn.MaxPool1d(kernel_size=3, stride=2, padding=1),

            # 第二个Inception块
            InceptionModule1D(128, 32, 32, 128, 16, 32, 64),
            nn.MaxPool1d(kernel_size=3, stride=2, padding=1),

            # 第三个Inception块
            InceptionModule1D(256, 48, 48, 192, 24, 48, 96),
            nn.MaxPool1d(kernel_size=3, stride=2, padding=1),

            # 第四个Inception块
            InceptionModule1D(384, 64, 64, 256, 32, 64, 128),
            nn.MaxPool1d(kernel_size=3, stride=2, padding=1),

            # 第五个Inception块
            InceptionModule1D(512, 64, 64, 256, 32, 64, 128),
...
def train_model(model, train_loader, val_loader, num_epochs=250):
    ...
    patience_counter = 0
    best_model_path = f"googlenet_best_model_{timestamp}.pth"
...
if __name__ == "__main__":
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    nin_model = GoogLeNet().to(device)
    print("=" * 33 + "网络权重初始化完毕" + "=" * 33)
    nin_model.apply(lambda m: init_param(m))
    print(f"GoogLeNet网络结构如下:{nin_model}")
