...
class ResidualBlock1D(nn.Module):
    """
    1D残差块（自动处理维度匹配）
    - 当stride≠1或in≠out时，自动创建1×1卷积下采样层
    - 严格遵循ResNet原始设计思想
    """
...
class ResNet(nn.Module):
    def __init__(self, profile_len=1024, param_dim=18):
        super().__init__()
        # 廓形分支 (2通道 x 1024)
        # ResNet风格,车轮、钢轨廓形对应两个通道,1D图像
        self.profile_branch = nn.Sequential(
            # 初始层：
            nn.Conv1d(2, 64, kernel_size=7, stride=2, padding=3, bias=False),  # [2,1024] → [32,512]
            nn.BatchNorm1d(64),
            ...
            # Stage 5:
            ResidualBlock1D(1024, 1024, stride=2),
            ResidualBlock1D(1024, 1024),

            # 全局池化 + 特征投影
            nn.AdaptiveAvgPool1d(8),
            nn.Flatten(),
            nn.Linear(8192, 2048),  # 降维
            nn.BatchNorm1d(2048),
            nn.ReLU(inplace=True),
            nn.Dropout(0.4)
        )
...
def train_model(model, train_loader, val_loader, num_epochs=250):
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode='min', factor=0.5, patience=15, min_lr=1e-7, threshold_mode='abs', threshold=min_delta
    )
...
if __name__ == "__main__":
    ...
    resnet_model.apply(lambda m: init_param(m))
    print(f"Alex网络结构如下:{resnet_model}")
    total, trainable = get_param_number(resnet_model)
    print(f"模型总参数量:{total}, 模型可训练参数量:{trainable}")
    # 训练模型
    train_model(resnet_model, train_loader, val_loader, num_epochs=250)
