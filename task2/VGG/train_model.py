...
class VGGNet(nn.Module):
    def __init__(self, profile_len=1024, param_dim=18):
        ...
        self.profile_branch = nn.Sequential(
            ...
            nn.AdaptiveAvgPool1d(8),  # 固定输出长度为8
            nn.Flatten()
        )
        # 参数分支 (18维)
...
def train_model(model, train_loader, val_loader, num_epochs=250):
    ...
    patience_counter = 0
    best_model_path = f"vgg_best_model_{timestamp}.pth"
...
if __name__ == "__main__":
    ...
    print("=" * 33 + "VGG网络建立完毕" + "=" * 33)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    vgg_model = VGGNet().to(device)
    print("=" * 33 + "网络权重初始化完毕" + "=" * 33)
    vgg_model.apply(lambda m: init_param(m))
    print(f"Alex网络结构如下:{vgg_model}")
    total, trainable = get_param_number(vgg_model)
    print(f"模型总参数量:{total}, 模型可训练参数量:{trainable}")
    # 训练模型
    train_model(vgg_model, train_loader, val_loader, num_epochs=300)
